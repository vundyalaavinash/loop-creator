...
def test_cli_call_returns_text():
    with patch("creator.adapters.devin.shutil.which", return_value="/usr/bin/devin"), \
         patch("creator.adapters.devin.subprocess.run", return_value=_mock_run("ok")):
...
def test_is_available_true_when_api_key_set():
    with patch("creator.adapters.devin.shutil.which", return_value=None):
